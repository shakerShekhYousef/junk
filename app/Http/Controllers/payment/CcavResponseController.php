...
use App\Models\PackagesSession;
...
class CcavResponseController extends Controller
{
    public function response()
    {
        ...
        if ($payment->order_status == "Success") {
            ...
            session()->flash('success', 'Purchase package ' . $payment->order_status);
            return redirect()->route('web_calander_data_show');
        }

        $order->delete();
        session()->flash('error', 'Purchase package ' . $payment->order_status);
    ...
    public function freezresponse()
    {
        ...
        if ($payment->order_status == "Success") {
            ...
            Mail::to($user->email)->send(new EmailAfterPaymentComplete($data));
            session()->flash('success', 'Purchase freeze ' . $payment->order_status);
            return redirect()->route('web_get_user_packages', $packagemember->member_id);
        }
        $order->delete();
        session()->flash('error', 'Purchase freeze ' . $payment->order_status);
        return redirect()->route('web_get_user_packages', $packagemember->member_id);
    }
    public function feesresponse()
    {
        ...
        if ($payment->order_status == "Success") {
            ...
            session()->flash('success', 'Purchase fee ' . $payment->order_status);
            return redirect()->route('web_get_user_fees', $fee->member_id);
        }
        $order->delete();
        session()->flash('error', 'Purchase fee ' . $payment->order_status);
        return redirect()->route('web_get_user_fees', $fee->member_id);
    }
