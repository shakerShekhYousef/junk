...
@section('head1')
    ...
    <!--<base href="">-->
    <meta charset="utf-8" />
    <title>Schedule – JUNK Fitness Dubai &mdash; JUNK</title>
...
@section('content')
    ...
    <div class="content-wrapper">
        <main class="Main Main--page">
            <div class="sche-section">
                <div class="row">
                    <div class="col-md-3 col-sm-4 col-4">
                        <div class="dropdown">
                            {{-- class music coach week direction --}}
                            <button class="dropbtn btn-event">CLASSES</button>
                            <div class="dropdown-content">
                                <a
                                    href="{{ route('back_calander_data_show', [0, session()->get('musicid', 0), session()->get('coachid', 0), session()->get('weekid', 0), 3]) }}">Classes</a>
                                @foreach ($classeslist as $class)
                                    <a
                                        href="{{ route('back_calander_data_show', [$class->id, session()->get('musicid', 0), session()->get('coachid', 0), session()->get('weekid', 0), 3]) }}">{{ $class->name }}</a>
                                @endforeach
                            </div>
                        </div>
                    </div>
                    {{-- <div class="col-md-3 col-sm-4 col-4">
                        <div class="dropdown">
                            <button class="dropbtn btn-music">MUSICS</button>
                            <div class="dropdown-content">
                                <a
                                    href="{{ route('back_calander_data_show', [session()->get('classid', 0), 0, session()->get('coachid', 0), session()->get('weekid', 0), 3]) }}">Musics</a>
                                @foreach ($musics as $music)
                                    <a
                                        href="{{ route('back_calander_data_show', [session()->get('classid', 0), $music->id, session()->get('coachid', 0), session()->get('weekid', 0), 3]) }}">{{ $music->title }}</a>
                                @endforeach
                            </div>
                        </div>
                    </div> --}}
                    <div class="col-md-3 col-sm-4 col-4">
                        <div class="dropdown">
                            <button class="dropbtn btn-music">COACHES</button>
                            <div class="dropdown-content">
                                <a
                                    href="{{ route('back_calander_data_show', [session()->get('classid', 0), session()->get('musicid', 0), 0, session()->get('weekid', 0), 3]) }}">Coaches</a>
                                @foreach ($coaches as $coach)
                                    <a
                                        href="{{ route('back_calander_data_show', [session()->get('classid', 0), session()->get('musicid', 0), $coach->id, session()->get('weekid', 0), 3]) }}">{{ $coach->username() }}</a>
                                @endforeach
                            </div>
                        </div>
                    </div>
                ...
                <div class="row pt-5">
                    <div class="col-md-6 col-6">
                        <button class="btn prev"
                            onclick="window.location='{{ route('back_calander_data_show') . '/' . session()->get('classid', 0) . '/' . session()->get('musicid', 0) . '/' . session()->get('coachid', 0) . '/' . ((int) session()->get('weekid', 0) - 1) . '/0' }}'">
                            <i class="fa fa-step-backward" aria-hidden="true"></i> Prev</button>
                    </div>
                    ...
                    <div class="col-md-6 col-6 text-right">
                        <button class="btn next"
                            onclick="window.location='{{ route('back_calander_data_show') . '/' . session()->get('classid', 0) . '/' . session()->get('musicid', 0) . '/' . session()->get('coachid', 0) . '/' . ((int) session()->get('weekid', 0) + 1) . '/2' }}'">Next
                            <i class="fa fa-step-forward" aria-hidden="true"></i></button>
                    </div>
                </div>
                ...
                <table class="table table-bordered">
                    <thead>
                        <!-- days -->
                        <tr>
                            @if ($currentdatename == $dates['sundate'])
                                <th style="text-align: center">
                                    <h5> Sun <br> {{ $dates['sundate'] }}</h5>
                                </th>
                            @else
                                <th style="text-align: center">Sun <br> {{ $dates['sundate'] }}</th>
                            @endif
                            @if ($currentdatename == $dates['mondate'])
                                <th style="text-align: center">
                                    <h5> Mon <br> {{ $dates['mondate'] }}</h5>
                                </th>
                            @else
                                <th style="text-align: center">Mon <br> {{ $dates['mondate'] }}</th>
                            @endif
                            @if ($currentdatename == $dates['tuedate'])
                                <th style="text-align: center">
                                    <h5> Tue <br> {{ $dates['tuedate'] }}</h5>
                                </th>
                            @else
                                <th style="text-align: center">Tue <br> {{ $dates['tuedate'] }}</th>
                            @endif
                            @if ($currentdatename == $dates['weddate'])
                                <th style="text-align: center">
                                    <h5> Wed <br> {{ $dates['weddate'] }}</h5>
                                </th>
                            @else
                                <th style="text-align: center">Wed <br> {{ $dates['weddate'] }}</th>
                            @endif
                            @if ($currentdatename == $dates['thudate'])
                                <th style="text-align: center">
                                    <h5> Thu <br> {{ $dates['thudate'] }}</h5>
                                </th>
                            @else
                                <th style="text-align: center">Thu <br> {{ $dates['thudate'] }}</th>
                            @endif
                            @if ($currentdatename == $dates['fridate'])
                                <th style="text-align: center">
                                    <h5> Fri <br> {{ $dates['fridate'] }}</h5>
                                </th>
                            @else
                                <th style="text-align: center">Fri <br> {{ $dates['fridate'] }}</th>
                            @endif
                            @if ($currentdatename == $dates['satdate'])
                                <th style="text-align: center">
                                    <h5> Sat <br> {{ $dates['satdate'] }}</h5>
                                </th>
                            @else
                                <th style="text-align: center">Sat <br> {{ $dates['satdate'] }}</th>
                            @endif
                    ...
                    <tbody>
                        ...
                        <tr>
                                ...
                                @foreach ($data['sun'] as $sun)
                                    <div style="text-align: center">
                                        ...
                                        <span
                                            ...
                                            <h6>{{ $sun['class_name'] }}</h6>
                                            <img src="{{ asset($sun['class_icon']) }}" alt="reCycle.png"
                                                width="40px" />
                                        </span><br>
                                        ...
                                        <p>{{ $sun['start_time'] }}</p>
                                        <p>{{ $sun['duration'] }}</p>
                                        <small>Users/Capacity/Availabe</small>
                                        <a href="{{ $sun['users'] }}">
                                            <span class="dot"
                                                style="text-align: center; background-color: rgb(207, 46, 34)"><b>{{ $sun['userscount'] }}</b></span>
                                        </a>
                                        <span class="dot mr-3 ml-2"
                                            style="text-align: center; background-color: rgb(199, 172, 22)"><b>{{ $sun['capacity'] }}</b></span>
                                        <span class="dot ml-2"
                                            style="text-align: center; background-color: rgb(29, 226, 72)"><b>{{ $sun['available'] }}</b></span>
                                    </div>
                                    <hr>
                                @endforeach
                            </td>
                            <!-- monday -->
                            <td>
                                @foreach ($data['mon'] as $mon)
                                    <div style="text-align: center">
                                        <span
                                            style="display: flex; justify-content: space-between;align-items: center;">
                                            <h6>{{ $mon['class_name'] }}</h6>
                                            <img src="{{ asset($mon['class_icon']) }}" alt="reCycle.png"
                                                width="40px" />
                                        </span><br>
                                        ...
                                        <a href="{{ $mon['users'] }}">
                                            <span class="dot mr-2 ml-2"
                                                style="text-align: center; background-color: rgb(207, 46, 34)"><b>{{ $mon['userscount'] }}</b></span>
                                        </a>
                                        <span class="dot mr-3 ml-2"
                                            style="text-align: center; background-color: rgb(199, 172, 22)"><b>{{ $mon['capacity'] }}</b></span>
                                        <span class="dot ml-2"
                                            style="text-align: center; background-color: rgb(29, 226, 72)"><b>{{ $mon['available'] }}</b></span>
                                    </div>
                                    <hr>
                                @endforeach
                            </td>
                            <!-- tuesday -->
                            <td>
                                @foreach ($data['tue'] as $tue)
                                    <div style="text-align: center">
                                        <span
                                            style="display: flex; justify-content: space-between;align-items: center;">
                                            <h6>{{ $tue['class_name'] }}</h6>
                                            <img src="{{ asset($tue['class_icon']) }}" alt="reCycle.png"
                                                width="40px" />
                                        </span><br>
                                        <p>{{ $tue['music_name'] }}</p>
                                        <p>{{ $tue['coach_name'] }}</p>
                                        ...
                                        <a href="{{ $tue['users'] }}">
                                            <span class="dot mr-2 ml-2"
                                                style="text-align: center; background-color: rgb(207, 46, 34)"><b>{{ $tue['userscount'] }}</b></span>
                                        </a>
                                        <span class="dot mr-3 ml-2"
                                            style="text-align: center; background-color: rgb(199, 172, 22)"><b>{{ $tue['capacity'] }}</b></span>
                                        <span class="dot ml-2"
                                            style="text-align: center; background-color: rgb(29, 226, 72)"><b>{{ $tue['available'] }}</b></span>
                                    </div>
                                    <hr>
                                @endforeach
                            </td>
                            <!-- wednesday -->
                            <td>
                                @foreach ($data['wed'] as $wed)
                                    <div style="text-align: center">
                                        <span
                                            style="display: flex; justify-content: space-between;align-items: center;">
                                            <h6>{{ $wed['class_name'] }}</h6>
                                            <img src="{{ asset($wed['class_icon']) }}" alt="reCycle.png"
                                                width="40px" />
                                        </span><br>
                                        <p>{{ $wed['music_name'] }}</p>
                                        <p>{{ $wed['coach_name'] }}</p>
                                        ...
                                        <a href="{{ $wed['users'] }}">
                                            <span class="dot mr-2 ml-2"
                                                style="text-align: center; background-color: rgb(207, 46, 34)"><b>{{ $wed['userscount'] }}</b></span>
                                        </a>
                                        <span class="dot mr-3 ml-2"
                                            style="text-align: center; background-color: rgb(199, 172, 22)"><b>{{ $wed['capacity'] }}</b></span>
                                        <span class="dot ml-2"
                                            style="text-align: center; background-color: rgb(29, 226, 72)"><b>{{ $wed['available'] }}</b></span>
                                    </div>
                                    <hr>
                                @endforeach
                            </td>
                            <!-- thursday -->
                            <td>
                                @foreach ($data['thu'] as $thu)
                                    <div style="text-align: center">
                                        <span
                                            style="display: flex; justify-content: space-between;align-items: center;">
                                            <h6>{{ $thu['class_name'] }}</h6>
                                            <img src="{{ asset($thu['class_icon']) }}" alt="reCycle.png"
                                                width="40px" />
                                        </span><br>
                                        <p>{{ $thu['music_name'] }}</p>
                                        <p>{{ $thu['coach_name'] }}</p>
                                        ...
                                        <a href="{{ $thu['users'] }}">
                                            <span class="dot mr-2 ml-2"
                                                style="text-align: center; background-color: rgb(207, 46, 34)"><b>{{ $thu['userscount'] }}</b></span>
                                        </a>
                                        <span class="dot mr-3 ml-2"
                                            style="text-align: center; background-color: rgb(199, 172, 22)"><b>{{ $thu['capacity'] }}</b></span>
                                        <span class="dot ml-2"
                                            style="text-align: center; background-color: rgb(29, 226, 72)"><b>{{ $thu['available'] }}</b></span>
                                    </div>
                                    <hr>
                                @endforeach
                            </td>
                            <!-- friday -->
                            <td>
                                @foreach ($data['fri'] as $fri)
                                    <div style="text-align: center">
                                        <span
                                            style="display: flex; justify-content: space-between;align-items: center;">
                                            <h6>{{ $fri['class_name'] }}</h6>
                                            <img src="{{ asset($fri['class_icon']) }}" alt="reCycle.png"
                                                width="40px" />
                                        </span><br>
                                        <p>{{ $fri['music_name'] }}</p>
                                        <p>{{ $fri['coach_name'] }}</p>
                                        ...
                                        <a href="{{ $fri['users'] }}">
                                            <span class="dot mr-2 ml-2"
                                                style="text-align: center; background-color: rgb(207, 46, 34)"><b>{{ $fri['userscount'] }}</b></span>
                                        </a>
                                        <span class="dot mr-3 ml-2"
                                            style="text-align: center; background-color: rgb(199, 172, 22)"><b>{{ $fri['capacity'] }}</b></span>
                                        <span class="dot ml-2"
                                            style="text-align: center; background-color: rgb(29, 226, 72)"><b>{{ $fri['available'] }}</b></span>
                                    </div>
                                    <hr>
                                @endforeach
                            </td>
                            <!-- saturday -->
                            <td>
                                @foreach ($data['sat'] as $sat)
                                    <div style="text-align: center">
                                        <span
                                            style="display: flex; justify-content: space-between;align-items: center;">
                                            <h6>{{ $sat['class_name'] }}</h6>
                                            <img src="{{ asset($sat['class_icon']) }}" alt="reCycle.png"
                                                width="40px" />
                                        </span><br>
                                        <p>{{ $sat['music_name'] }}</p>
                                        <p>{{ $sat['coach_name'] }}</p>
                                        ...
                                        <a href="{{ $sat['users'] }}">
                                            <span class="dot mr-2 ml-2"
                                                style="text-align: center; background-color: rgb(207, 46, 34)"><b>{{ $sat['userscount'] }}</b></span>
                                        </a>
                                        <span class="dot mr-3 ml-2"
                                            style="text-align: center; background-color: rgb(199, 172, 22)"><b>{{ $sat['capacity'] }}</b></span>
                                        <span class="dot ml-2"
                                            style="text-align: center; background-color: rgb(29, 226, 72)"><b>{{ $sat['available'] }}</b></span>
                                    </div>
                                    <hr>
                                @endforeach
                            </td>
...
@section('custom-script')
    <script src="https://static1.squarespace.com/static/ta/55f0a9b0e4b0f3eb70352f6d/345/scripts/site-bundle.js"
    ...
    <script type="text/javascript">
        ...
        function myFunction() {
            if (window.pageYOffset > sticky) {
                document.getElementById("free-class").style.backgroundColor = "#8fd241";
                document.getElementById("free-class").style.color = "rgb(0 0 0)";
                document.getElementById("free-class").style.padding = "10px 20px";
